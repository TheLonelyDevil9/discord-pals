...
def _parse_other_prompt_sections(content: str) -> dict[str, str]:
    ...
    sections = {}
    matches = []
    for match in RE_SECTION_HEADING.finditer(content):
        normalized_name = _normalize_section_name(match.group(1))
        canonical_name = OTHER_PROMPT_SECTION_ALIASES.get(normalized_name)
        if canonical_name:
            matches.append((match, canonical_name))

    for index, (match, canonical_name) in enumerate(matches):
        start = match.end()
        end = matches[index + 1][0].start() if index + 1 < len(matches) else len(content)
        sections[canonical_name] = (content[start:end] or "").strip()
    return sections
...
class CharacterManager:
    ...
    def build_other_prompt(
        ...
        return self.prompt_manager.build_other_prompt(section_name, replacements, now=now)
